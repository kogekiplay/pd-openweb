...
// graph 是 Set，展开后元素推成 unknown，标一下才能用字符串方法
const lessInGraph = [...graph].filter((f: string) => f.endsWith('.less'));

assert.ok(
  lessInGraph.some((f: string) => f.endsWith('themes/antd-color.less')),
