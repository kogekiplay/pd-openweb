...
function collect(dir, out) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (EXTS.has(path.extname(entry.name)) && !/\.spec\.(js|ts)$/.test(entry.name)) {
      // 【两个后缀都要排】spec 自己改成 .ts 之后，只排 .spec.js 会让这条 spec
      // 扫到【它自己】第 44 行那句 require('antd')，报出一条指向自身的假违规。
      out.push(abs);
    }
  }
