...
<script src="webpack[singleExtractModules]?src/pages/Mobile/index.tsx"></script>
```

模块引用按照上述格式，问号后未模块入口相对路径。 []方括号内为页面类型，主站内的都是 index ，需要分发出去的独立入口用 single 例如：`webpack[single]?src/pages/PublicWorksheet/`
用 singleExtractModules 例如：`webpack[singleExtractModules]?src/pages/Mobile/index.tsx`
