      ...
      <FullCalendar
        ...
        moreLinkContent={info => {
          /* 【带上「更多」两个字】光一个 "+38" 看不出是能点的；样式那边同时把它做成了胶囊。
             ...
             【「更多」单独包一层 span】周/日视图里这个链接是**浮在事件上面**的：库给它的
             容器是 `left:24px; right:0`，而事件本身占满整列（2~90），两者必然重叠，
             挪位置解决不了（右贴、下沉都试过，最多换一个被压的对象）。
             唯一真正有效的是把它变窄 —— 时间格里只留 "+94"，宽度 60 -> 36，
             事件能露出来的文字从 26px 变成 53px，名字就看得见了。
             月视图和弹层里空间够，仍然显示完整的「+N 更多」。 */
          return (
            <div title={_l('查看其他%0个', info.num)}>
              {`+${info.num}`}
              <span className="moreWord">{` ${_l('更多')}`}</span>
            </div>
          );
        }}
        moreLinkClick={() => {
