...
export const formatControlsBySectionId = controls => {
  let list = [];
  const data = controls.filter((o: FormControl) => !o.sectionId);
  const dataChild = controls.filter((o: FormControl) => !!o.sectionId);
  data.map(o => {
...
export const formatControlsChildBySectionId = controls => {
  ...
  list.forEach(o => {
    o.child = controls
      .filter((it: FormControl) => it.sectionId === o.controlId)
...
export const getSectionId = controls => {
  const sectionIds = _.uniq(controls.map((o: FormControl) => o.sectionId).filter(o => !!o));
  return sectionIds;
};
...
export const getRealData = (control, controls, allControls, isAdd) => {
  ...
  if (control.sectionId) {
    //新增
    if (isAdd) {
      let parent = controls.find((o: FormControl) => o.controlId === control.sectionId);

      //已存在父级
      if (parent) {
        return controls.concat([control]);
      } else {
        return controls.concat([allControls.find((o: FormControl) => o.controlId === control.sectionId), control]);
      }
    } else {
      //删减
      let childs = controls.filter((o: FormControl) => o.sectionId === control.sectionId);

      //只剩唯一子集，则包括父级一起删除
      if (childs.length <= 1) {
        return controls.filter((o: FormControl) => o.controlId !== control.controlId && o.controlId !== control.sectionId);
      } else {
        return controls.filter((o: FormControl) => o.controlId !== control.controlId);
      }
    }
  } else {
    //不是子集
    let childs = allControls.filter((o: FormControl) => (o.sectionId ? o.sectionId === control.controlId : false));

    if (isAdd) {
      return controls.concat([control, ...childs]);
    } else {
      //删减
      return controls.filter((o: FormControl) => ![...childs.map(it => it.controlId), control.controlId].includes(o.controlId));
    }
  }
};
