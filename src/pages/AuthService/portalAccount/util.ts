...
export const getCurrentId = cb => {
  ...
  let href = decodeURIComponent(String(ReturnUrl ? ReturnUrl : location.href));
  let currentAppId = '';
  urlList.forEach(o => {
    if (href.indexOf(o) >= 0) {
...
export const getCurrentExt = (appId, suffix) => {
  ...
  const urlPathname = new URL(decodeURIComponent(String(ReturnUrl ? ReturnUrl : location.href)));
