...
export default function AddAndEditAppAccess(props) {
  ...
  const onSubmit = () => {
    ...
    const params = {
      ...
      limitAction,
      whiteApps: limitAction === 1 ? appList.map(({ appId }: { appId?: string; [key: string]: any }) => ({ appId })) : [],
      blackApps: limitAction === 0 ? appList.map(({ appId }: { appId?: string; [key: string]: any }) => ({ appId })) : [],
      isAllowPublicAccess,
      isAllowCrossApp,
