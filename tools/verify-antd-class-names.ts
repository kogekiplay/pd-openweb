 ...
 *     node tools/verify-antd-class-names.ts
 */
const fs = require('fs');
 ...
 *
 * 单独运行：node tools/verify-antd-class-names.ts --static
 * ==========================================================================*/
...
async function classNamesFor(antdPath) {
  ...
  for (const [name, render] of Object.entries(cases) as [string, any][]) {
...
(async () => {
  ...
  for (const [name, r] of Object.entries(v4) as [string, any][]) {
  ...
  for (const [name, r] of Object.entries(v6) as [string, any][]) {
