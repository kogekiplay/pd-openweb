 ...
 *     node tools/verify-cm6-formulamarks.ts
 *
 * 注意：等 CM5 被彻底删掉之后，这个脚本的「参照物」也就没了。届时应把当时的比对结果
 * 冻结成期望值（或直接删掉本脚本，只留 tools/verify-cm6-functioneditor.ts 那份行为测试）。
 */
const fs = require('fs');
...
//   CM5_LIB=/tmp/cm5-lib node tools/verify-cm6-formulamarks.ts --write-fixture
    ...
    JSON.stringify(
      {
        _note:
          ...
          '继续守住「新实现与 CM5 逐字一致」。重新生成方式见 tools/verify-cm6-formulamarks.ts 文件头。',
