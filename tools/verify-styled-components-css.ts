 ...
 *   cd <repo> && SC_NEW=/tmp/sc6/node_modules/styled-components node tools/verify-styled-components-css.ts
 *
 * 升级落地后这个脚本就没有「旧版」参照物了，届时同样要把结果冻结成 fixture
 * （做法见 tools/verify-rc-trigger-parity.ts 文件头）。
 */
const fs = require('fs');
  ...
  if (oldVer === newVer) {
    console.error(
      `两侧都是 ${oldVer}，没有可比的对象。\n` +
        ...
        '  cd <repo> && SC_OLD=/tmp/sc4/node_modules/styled-components node tools/verify-styled-components-css.ts\n' +
        '日常防回归请用 tools/scan-styled-missing-semicolon.ts（它不依赖旧版本）。',
    );
    process.exit(2);
  }
}
