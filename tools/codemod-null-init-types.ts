 ...
 *
 * 用法：
 *   node tools/codemod-null-init-types.ts --list
 *   node tools/codemod-null-init-types.ts
 */
const fs = require('fs');
...
for (const sf of program.getSourceFiles()) {
  ...
  for (const decl of decls) {
    ...
    // types 要显式写成 Set<string>，否则展开后是 unknown[]，正则测不了
    const rec = { sf, name, types: new Set<string>(), dirty: false, writes: 0, pos: decl.name.end };
    (function walk(n) {
      if (
