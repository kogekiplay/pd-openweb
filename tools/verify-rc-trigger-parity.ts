 ...
 *   JSDOM_PATH=/tmp/jsdom-for-verify/node_modules/jsdom node tools/verify-rc-trigger-parity.ts
 *
 ...
 *     node tools/verify-rc-trigger-parity.ts --write-fixture
 */
// 这个校验脚本跑在 jsdom 里，下面这些浏览器全局由 jsdom 注入。
  ...
  if (WRITE_FIXTURE) {
    ...
    ref = {
      _note:
        ...
        '消失之后继续守住「5.x 的渲染与行为与它一致」。重新生成方式见 tools/verify-rc-trigger-parity.ts 文件头。',
