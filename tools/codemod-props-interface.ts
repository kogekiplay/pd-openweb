 ...
 *
 * 【为什么不能直接用 InferProps】调研见 tools/survey-proptypes-coverage.ts：
 ...
 *    tools/survey-proptypes-coverage.ts 那份调研只量了【组件内部】的覆盖率，
 *    漏掉了调用点这一维。
 *
 ...
 *
 * 用法（仅供调研）：node tools/codemod-props-interface.ts --list
 */
const fs = require('fs');
...
function render(name, declared, used, needsIndex) {
  ...
  lines.push(' * 只按它生成会让用到未声明 prop 的地方报 TS2339，见 tools/survey-proptypes-coverage.ts）。');
