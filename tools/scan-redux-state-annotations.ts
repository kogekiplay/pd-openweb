...
// 逐条挂上去的统计：key -> { count, files }
const stats: Record<string, { count: number; files: Set<string> }> = {};
...
// stats 是逐条挂上去的统计对象，Object.entries 给的是 [string, unknown][]
const rows = Object.entries(stats).sort(
  (a: [string, any], b: [string, any]) => b[1].count - a[1].count,
);
...
if (bare) {
  ...
  console.log('\nbare 站点的文件扩展名分布：');
  for (const [e, n] of Object.entries(byExt).sort((a: [string, number], b: [string, number]) => b[1] - a[1])) console.log(`  ${String(n).padStart(3)}  ${e}`);
}
