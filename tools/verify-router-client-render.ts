 ...
 *
 * 为什么在 verify-router-render.ts 之外还要一份：
 ...
 *
 * 运行：
 *   JSDOM_PATH=/tmp/jsdom-for-verify/node_modules/jsdom node tools/verify-router-client-render.ts
 */
const fs = require('fs');
...
// 【裸标识符 require】的理由见 verify-router-render.ts 文件头：
...
const stubConfig = (cfg, prefix = '') => {
  ...
  for (const [key, r] of Object.entries(cfg) as [string, any][]) {
    out[key] = {
      ...r,
...
// tools/verify-router-matching.ts 的例外表放行了 67 条顶栏差异，前提就是这个
