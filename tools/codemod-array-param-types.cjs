 ...
 *      codemod-domain-callback-params.cjs 里那张一致（那张已经过一轮差分闸门验证）。
 *
 * 两条都不成立就【不动】—— 不写 any[] 充数。
 *
 * 只处理 `= []`。`= {}` 不处理：调用点传的是各式对象字面量，求并集要么超长要么
 * 触发多余属性检查（TS2353），codemod-props-interface.cjs 已经在这上面栽过一次。
 *
 * 用法：
 *   node tools/codemod-array-param-types.cjs --list
 *   node tools/codemod-array-param-types.cjs
 */
const fs = require('fs');
...
for (const [file, bucket] of [...byFile].sort()) {
  ...
  const need = [...new Set(bucket.ins.map(i => i.type).filter(t => DOMAIN_TYPES.has(t)))]
    .map(t => t.replace('[]', ''))
