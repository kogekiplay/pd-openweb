 ...
 *
 * 运行（RR4 默认在 /tmp/rr4，见 verify-router-matching.ts 文件头）：
 *   node tools/verify-without-url-predicates.ts
 */
const fs = require('fs');
...
for (const url of urls) {
  // 异构元组，不标的话推成 (string | Function)[]，oldFn 就不可调用
  for (const [label, oldFn, newFn] of [
    ['顶栏', oldHeader, newHeader],
    ['聊天栏', oldChat, newChat],
  ] as [string, (u: any) => any, (u: any) => any][]) {
    const o = oldFn(url);
    const n = newFn(url);
