 ...
 *   cd <repo> && RR7=/tmp/rr7/node_modules/react-router-dom node tools/verify-router-matching.ts
 *
 ...
 *   cp tools/verify-router-matching.ts /tmp/rr-baseline/tools/
 *   ln -s <repo>/node_modules /tmp/rr-baseline/node_modules
 *   cd /tmp/rr-baseline && JSDOM_PATH=... node tools/verify-router-matching.ts --write-fixture
...
function loadConfigs() {
  ...
  for (const [label, file, exportName] of CONFIG_FILES) {
    ...
    for (const [name, table] of candidates) {
      ...
      const routes = (Object.entries(table) as [string, any][])
        .filter(([, r]: [string, any]) => r && (typeof r.path === 'string' || Array.isArray(r.path)))
...
if (process.argv.includes('--write-fixture')) {
  fixture._note =
    ...
    '重新生成方式见 tools/verify-router-matching.ts 文件头。';
