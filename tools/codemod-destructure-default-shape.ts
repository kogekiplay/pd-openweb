 ...
 * （和 codemod-domain-destructure-types.ts 同一条约束，那边栽过。）
 *
 * 【只管变量声明的解构，不碰解构形参】形参标上去会让属性变必填，
 * 所有不传它的调用点齐刷刷 TS2741。原因详见
 * codemod-domain-destructure-types.ts 的头注释。
 *
 * 用法：
 *   node tools/codemod-destructure-default-shape.ts --list
 *   node tools/codemod-destructure-default-shape.ts
 */
const fs = require('fs');
