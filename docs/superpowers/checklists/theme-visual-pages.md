...
> 唯一的例外是**颜色取值本身**：`src/common/theme/palette.spec.ts` 守运行期算出来的
> 主色调色板，`src/common/mdcss/themes/functionalTextTokens.spec.ts`（2026-09-22 加）
> 直接读两个主题的 Less 源文件，算 success / warning / error 三个文字档和两个 hover 档
> 对页面底、卡片底、自家浅底的对比度。它们测的是「这个值够不够」，
> **不测「这个值有没有用对地方」** —— 后者仍然只有这份清单。
